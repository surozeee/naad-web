...
const LIST_PATH = '/user/role/list';
...
/** GET list roles (query params) - legacy. Prefer POST. */
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[API roles]', e);
    return NextResponse.json({ message: 'Failed to fetch roles' }, { status: 500 });
  }
}

/** POST list roles (paginated). Body: { pageNo, pageSize, sortBy?, sortDirection?, search? }. Forwards to backend GET list with query params. */
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[API roles POST]', e);
    return NextResponse.json({ message: 'Failed to fetch roles' }, { status: 500 });
  }
}
